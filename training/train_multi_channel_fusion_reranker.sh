...
if [[ $# -ge 13 ]]; then
    ...
    reranker_feats=${13}  # eo-feats, lm-feats, eo+lm
    char_lm_url=${14}
    if [[ "${reranker_feats}" == "lm-feats" || "${reranker_feats}" == "eo+lm" ]]; then
